                ...
                'click': function() {
                    if (!running) {
                        ...
                        fs.exec_direct('speedtest', command).then(function(response) {
                            ...
                            var date = currentDate.toLocaleDateString(undefined, {
                                weekday: 'short',
                                month: 'short',
                                day: 'numeric'
                            });
                            ...
                            var ping = result.ping.latency ? result.ping.latency + ' ms' : '-';
                            var jitter = result.ping.jitter ? result.ping.jitter + ' ms' : '-';
                            var download = result.download.bandwidth ? (result.download.bandwidth / 1000000) + ' Mbit/s' : '-';
                            var upload = result.upload.bandwidth ? (result.upload.bandwidth / 1000000) + ' Mbit/s' : '-';
                            document.getElementById('client-ip').textContent = result.interface.externalIp;
                            document.getElementById('client-isp').textContent = result.isp;
                            document.getElementById('server-name').textContent = result.server.name;
                            document.getElementById('server-id').textContent = result.server.id;
                            document.getElementById('server-location').textContent = result.server.location;
                            ...
                            running = false;
                            var resultString = '| ' + date + ' | ' + time + ' | ' + value + ' | ' + ping + ' | ' + jitter + ' | ' + download + ' | ' + upload + ' |\n';
